...
const CreateAstronomic = async(req, res)=>{
    ...
    if (checkAstronomic) {
        return res.status(400).json({message:"Astronomic Details already exist"})
    }

    const data = await AstronomicModel.create({ProfileID, ...validateData.data})
    return res.status(200).json({ message: "Astronomic Details Created Succesfull", data })
...
const UpdateAstronomic = async(req, res)=>{
   ...
   try {
     const data = await AstronomicModel.findOneAndUpdate({ProfileID}, {ProfileID, ...validateData.data}, {new:true})
