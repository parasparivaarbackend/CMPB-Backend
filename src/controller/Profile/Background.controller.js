...
const CreateBackground = async(req, res)=>{
    ...
    const data = await BackgroundModel.create({ProfileID, ...validateData.data})

    return res.status(200).json({message:"Backgound Created Succesfull", data})
...
const UpdateBackground = async(req, res)=>{
    ...
    const data = await BackgroundModel.findOneAndUpdate({ProfileID}, {ProfileID, ...validateData.data}, {new:true})

    return res.status(200).json({ message:"Backround Details Updated Succesfull", data})
