...
const PartnerExpectationSchema = z.object({
    ...
    SmokingAcceptable: z.boolean(),
    DrinkAcceptable: z.boolean(),
    DietAcceptable: z.boolean(),
...
const CreatePartnerExpectation = async(req, res)=>{
    ...
    const checkPartnerExpectaion = await PartnerExpectationModel.findOne({ ProfileID });
    if (checkPartnerExpectaion) {
        return res.status(400).json({ message: "Partner Expectation already exist" })
    }

    const data = await PartnerExpectationModel.create({ProfileID, ...validaData.data})
    return res.status(200).json({ message:"Partner Expectation Created Succesfull", data })
...
const UpdatePartnerExpectation = async(req, res)=>{
    ...
    const data = await PartnerExpectationModel.findOneAndUpdate({ ProfileID }, { ProfileID, ...validateData.data }, { new: true });
    return res.status(200).json({ message: "Partner Expectation Updated Succesfull", data })
}
