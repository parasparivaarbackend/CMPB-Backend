...
const CreateLanguages = async(req, res)=>{
    ...
    return res.status(200).json({mesaage:"Languages created Succesfull", data})
...
const UpdateLanguages = async(req, res)=>{
    ...
    const data = await LanguageModel.findOneAndUpdate({ProfileID}, {...validateData.data}, {new:true})

    return res.status(200).json({mesaage:"Language Updated Succesfull", data})
}
