...
const CreateResidencyInfo = async(req, res)=>{
    ...
    if (checkResidencyInfo) {
        return res.status(400).json({ message: "Residency Information already exist" })

    }

    const data = await residencyInfosModal.create({ProfileID, ...validateData.data})

    return res.status(200).json({message:"Residency Information Created Succesfull", data})
}
...
const UpdateResidencyInfo = async(req, res)=>{
    ...
    try {
        const updateData = await residencyInfosModal.findOneAndUpdate({ ProfileID }, { ...validateData.data }, { new: true })
        return res.status(200).json({ message: "Residency Information Updated Succesfull", updateData });
    } catch (error) {
        console.log(error);
