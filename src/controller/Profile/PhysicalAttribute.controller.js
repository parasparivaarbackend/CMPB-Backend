...
const CreatePhysicalAttribute = async(req, res)=>{
    ...
    const validateData = validatePhysicalAttribute.safeParse(PhysicalAttributeData)
    ...
    const data = await PhysicalAttributeModel.create({ProfileID, ...validateData.data})
     return res.status(200).json({message:"PhysicalAttribute Created Successfull", data})
...
const UpdatePhysicalAttribute = async(req, res)=>{
    ...
    try {
        const data = await PhysicalAttributeModel.findOneAndUpdate({ProfileID}, {...validateData.data}, {new:true})
