...
const ChurayeHuaPalGet = async(req, res)=>{
    ...
    return res.status(200).json({ message:"Churaye Hua Pal get Succesfull", data})
}
...
const ChurayeHuaPalCreate = async (req, res) => {
    ...
    try {
        ...
        return res.status(200).json({ message:"Churaye Hua Pal is Created Succesfull", data })
    } catch (error) {
        console.log(error);
...
const ChurayeHuaPalUpdate = async(req, res)=>{
   ...
   try {
     const data = await ChurayeHuaPalModel.findByIdAndUpdate(id, {...validateData.data}, {new:true})
     if (!data) {
         return res.status(400).json({ message: "Data Not Found", data })
     }
       return res.status(400).json({ message:"Churaye Hua Pal is Updated Succesfull", data })
   } catch (error) {
    console.log(error);
...
const ChurayeHuaPalDelete = async(req, res)=>{
    ...
    return res.status(200).json({ message: "Churaye Hua Pal Deleted Succesfull" })

}
export { ChurayeHuaPalCreate, ChurayeHuaPalUpdate, ChurayeHuaPalDelete, ChurayeHuaPalGet }