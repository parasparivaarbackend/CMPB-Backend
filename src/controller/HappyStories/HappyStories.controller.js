...
const GetHappyStories = async (req, res) => {
  ...
  return res
    .status(200)
    .json({ message: "Happy Stories get Succesfull", data });
};
...
const CreateHappyStories = async (req, res) => {
  ...
  try {
    ...
    const data = await happystoriesmodel.create({
      ...validateData.data,
      story: imageData,
    });
    return res
      .status(200)
      .json({ message: "Happy Story Created Ssuccesfull", data });
  } catch (error) {
    console.error(error);
  }
};
...
const UpdateHappyStories = async (req, res) => {
  ...
  try {
    ...
    return res
      .status(400)
      .json({ message: "Something wrong try again", error });
  }
};
