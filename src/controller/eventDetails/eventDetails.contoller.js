...
const CreateEventDetails = async(req, res)=>{
    ...
    const checkDate = await eventdetails.findOne({ availableDates: createData.availableDates })
    if (checkDate) {
...
const UpdateEventDetails = async(req, res)=>{
    ...
    const data = await eventdetails.findOneAndUpdate({ _id:id }, {...validateData.data}, {new:true})
    if (!data) {
